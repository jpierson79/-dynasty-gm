...
20. Controlled production acceptance defaults to one to three eligible UUIDs from an empty-by-default review set. An expanded acceptance batch of four to ten UUIDs is permitted only when an architect explicitly approves a dedicated gate and the active `docs/NEXT_TASK.md` states the exact task-level batch size. `NEXT_TASK.md` alone cannot silently raise the limit: the gate must also rely on a deployed and accepted durable audit/recovery boundary, deployed and accepted expanded-cap database constraints, a league-scoped opt-in, Current-period-only data, an accepted season context, exact player and team identity, manual-override protection, stale-preview protection, a deterministic manifest and digest, replay prevention, and pre/post protected-field verification. Release candidates remain excluded unless that same gate separately and explicitly authorizes them. Expanded authority is gate-specific, applies only to the stated batch, and never carries forward to another task or batch. If any prerequisite, approval, batch size, or repository document conflicts or is ambiguous, fail closed to the one-to-three default. For every default or expanded acceptance, any selection change invalidates acknowledgement; the final confirmation must name the selected players; and the repository must receive only that revalidated subset.
