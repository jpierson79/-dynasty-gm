...
function renderImportPreview(step,preview){
  ...
  el.innerHTML=`<h3>${clean(importLabels[step])} Preview</h3>${countGridHTML(fantraxCounts)}<p class="note mt-10"><b>Detected columns:</b> ${columns||"JSON file"}</p>${fantraxTokens}${rosterDiagnostics}${statcastIdentity}${errors}${warnings}${categories}${duplicateGroups}${ambiguous}`;
}
