...
assert.match(importService,/actionAttempted:item\.category===HKB_DIAGNOSTIC\.NON_PLAYER_ASSET\?"Preserve non-player valuation asset":"Update existing player HKB values"/);
assert.match(importService,/diagnosticCategory:item\.category/);
assert.match(authUi,/HKB Matching Diagnostics/);
