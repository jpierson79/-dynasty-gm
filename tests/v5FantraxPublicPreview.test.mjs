...
const ownershipConflictPreview=compareFantraxPreview(responses,{players:[{...cloudPlayers[0],roster_status_source:"UNKNOWN",roster_status_override_at:null,roster_status_override_by:null,owner_team_id:"different-team"},...cloudPlayers.slice(1)],teams:[...cloudTeams,{id:"different-team",name:"Different Cloud Team"}]});
assert.equal(ownershipConflictPreview.rosterItems[0].ownershipDifference,true);
assert.equal(ownershipConflictPreview.rosterItems[0].ownershipConflictType,"DIFFERENT_CLOUD_OWNER");
assert.equal(ownershipConflictPreview.rosterItems[0].currentOwnerTeamName,"Different Cloud Team");
assert.equal(ownershipConflictPreview.rosterItems[0].futureSyncRecommendation,"REVIEW_CONFLICT","ownership differences must never enter the automatic status apply set");
assert.match(ownershipConflictPreview.rosterItems[0].diagnosticReason,/Cloud owner Different Cloud Team differs from Fantrax team Alpha/i);
const freeAgentConflictPreview=compareFantraxPreview(responses,{players:[{...cloudPlayers[0],roster_status_source:"UNKNOWN",roster_status_override_at:null,roster_status_override_by:null,owner_team_id:null},...cloudPlayers.slice(1)],teams:cloudTeams});
assert.equal(freeAgentConflictPreview.rosterItems[0].ownershipConflictType,"CLOUD_FREE_AGENT");
assert.match(freeAgentConflictPreview.rosterItems[0].diagnosticReason,/free agent/i);
...
assert.match(view,/Cloud free-agent conflicts/);
assert.match(view,/Different-owner conflicts/);
assert.match(view,/currentOwnerTeamName\|\|"Free agent"/);
