...
export function createGate4AcceptanceHarness({artifactCommit="",persistenceAuthority=false,dependencies={}}={}){
  ...
  const state={artifactCommit:clean(artifactCommit),checkpoints:{},candidateIds:[],previewA:null,previewB:null,manifest:null,manifestDigest:"",humanDigest:"",armedContextDigest:"",persistenceAuthority:Boolean(persistenceAuthority),persistenceAvailable:false,persistenceExecutable:false,armed:false,persistenceCalled:false};
  ...
  const invalidateFrom=name=>{const index=GATE4_CHECKPOINTS.indexOf(name);GATE4_CHECKPOINTS.slice(index).forEach(key=>delete state.checkpoints[key]);state.armed=false;state.armedContextDigest="";state.humanDigest="";refreshPersistenceState()};
