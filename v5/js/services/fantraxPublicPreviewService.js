...
export function compareFantraxPreview(responses,{players=[],teams=[]}={}){
  ...
  const rosterItems=(rosterSource.rosterItems||[]).map(item=>{
    ...
    const cloudPlayer=playerMatch.matchedPlayer||null,currentOwnerTeamId=clean(cloudPlayer?.owner_team_id),currentRosterStatus=upper(cloudPlayer?.roster_status)||"UNCLASSIFIED";
    ...
    const futureSyncRecommendation=normalizedRosterStatus==="UNCLASSIFIED"?"REVIEW_CONFLICT":!rosterStatusDifference?"NO_CHANGE":activeManualOverride?"PRESERVE_MANUAL_OVERRIDE":"APPLY_FANTRAX_STATUS";
    return {...item,normalizedRosterStatus,currentStatusSource,activeManualOverride,fantraxConflict:rosterStatusDifference,futureSyncRecommendation,...playerMatch,playerIdentityResult:playerMatch.identityResult,matchedTeamUuid:teamMatch.matchedTeamUuid||"",matchedTeam:teamMatch.matchedTeam||null,teamIdentityResult:teamMatch.identityResult,teamDiagnosticReason:teamMatch.diagnosticReason,currentOwnerTeamId,currentRosterStatus,ownershipDifference:Boolean(playerMatch.matchedPlayerUuid&&teamMatch.matchedTeamUuid&&!same(currentOwnerTeamId,teamMatch.matchedTeamUuid)),rosterStatusDifference,diagnosticReason:[playerMatch.diagnosticReason,teamMatch.diagnosticReason].filter(Boolean).join(" ")};
  });
