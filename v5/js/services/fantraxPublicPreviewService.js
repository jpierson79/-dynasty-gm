...
export function compareFantraxPreview(responses,{players=[],teams=[]}={}){
  ...
  const rosterItems=(rosterSource.rosterItems||[]).map(item=>{
    ...
    const normalizedRosterStatus=normalizePublicRosterStatus(item.sourceStatus);
    return {...item,normalizedRosterStatus,...playerMatch,playerIdentityResult:playerMatch.identityResult,matchedTeamUuid:teamMatch.matchedTeamUuid||"",matchedTeam:teamMatch.matchedTeam||null,teamIdentityResult:teamMatch.identityResult,teamDiagnosticReason:teamMatch.diagnosticReason,currentOwnerTeamId,currentRosterStatus,ownershipDifference:Boolean(playerMatch.matchedPlayerUuid&&teamMatch.matchedTeamUuid&&!same(currentOwnerTeamId,teamMatch.matchedTeamUuid)),rosterStatusDifference:Boolean(playerMatch.matchedPlayerUuid&&currentRosterStatus!==normalizedRosterStatus),diagnosticReason:[playerMatch.diagnosticReason,teamMatch.diagnosticReason].filter(Boolean).join(" ")};
  });
