...
export async function runDataHealth(leagueId,{teamId="",authenticatedUserId="",preferredTeamId="",userTeamResolution=null,tradeState={},rosterStatusManager=null,fantraxPreview=null,fantraxSyncAttempts=null,fantraxSyncAuditStatus="UNAVAILABLE",fantraxSyncAuditError="",mlbamBackfillPreview=null}={}){
  ...
  const checks=[
    ...
    {name:"Automated Statcast Coordinated Session Outcome",status:["FAILED","UNAVAILABLE","QUERY_FAILED","BLOCKED"].includes(statcastHealth.session?.status)?"FAIL":["PARTIAL","NEVER_RUN","NOT_RUN","RUNNING"].includes(statcastHealth.session?.status)?"WARNING":"PASS",details:detail("Automated Statcast Coordinated Session Outcome",[statcastHealth.session||{status:"UNAVAILABLE"}])},
    ...
    {name:"Automated Statcast Identity And Row Outcomes",status:statcastHealth.status==="UNAVAILABLE"?"FAIL":Object.values(statcastHealth.types||{}).some(type=>Number(type.failed)>0)?"WARNING":"PASS",details:detail("Automated Statcast Identity And Row Outcomes",[statcastHealth])},
    ...
    {name:"MLBAM Backfill Reason Accounting",status:mlbamHealth.status==="AVAILABLE"&&Object.values(mlbamHealth.reasonCounts||{}).reduce((sum,count)=>sum+Number(count||0),0)===mlbamHealth.total?"PASS":mlbamHealth.status==="AVAILABLE"?"FAIL":"WARNING",details:detail("MLBAM Backfill Reason Accounting",[{reasonCounts:mlbamHealth.reasonCounts||{},total:mlbamHealth.total}])},
    {name:"MLBAM Hypothetical Statcast Coverage",status:Object.values(mlbamHealth.hypotheticalStatcast||{}).some(value=>value?.status==="AVAILABLE")||mlbamHealth.hypotheticalStatcast?.status==="AVAILABLE"?"PASS":"WARNING",details:detail("MLBAM Hypothetical Statcast Coverage",[mlbamHealth.hypotheticalStatcast||{status:"UNAVAILABLE"}])},
