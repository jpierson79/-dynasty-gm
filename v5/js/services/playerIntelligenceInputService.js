...
export function buildCanonicalPlayerIntelligenceInput({player,league,metricRows=[],season,asOfDate=new Date().toISOString(),staleAfterMs=48*60*60*1000}={}){
  ...
  const roleEvidence={mlbStatus:player.is_minor_leaguer?"MINORS":player.mlb_team?"MLB":"UNKNOWN",rosterStatus:player.roster_status??null,pitcherEligibility:positionEligibility.eligible.filter(item=>item==="SP"||item==="RP"),everydayRole:null,rotationRole:null,bullpenRole:null,savesHoldsOpportunity:null,status:player.roster_status||player.mlb_team||player.is_minor_leaguer?INPUT_AVAILABILITY.AVAILABLE:INPUT_AVAILABILITY.UNAVAILABLE};
  ...
  return {schema:"player-intelligence-input-v1",inputVersion:PLAYER_INTELLIGENCE_INPUT_VERSION,playerId:player.id,leagueId:league.id,asOfDate:new Date(asOfDate).toISOString(),season:season??null,
    player:{id:player.id,name:player.name??null,mlbamId:player.mlbam_id??null,fantraxId:player.fantrax_id??null,fantraxApiPlayerId:player.fantrax_api_player_id??null,age:player.age??null,organization:player.mlb_team??null,isMinorLeaguer:Boolean(player.is_minor_leaguer),rosterStatus:player.roster_status??null,ownerTeamId:player.owner_team_id??null,isFreeAgent:player.is_free_agent??player.owner_team_id===null},
    leagueContext:leagueContext(league,season),positionEligibility,
    production:{status:INPUT_AVAILABILITY.UNAVAILABLE,fantasyPoints:null,gamesPlayed:null,plateAppearances:raw.pa??null,inningsPitched:null,appearances:null,season:season??null,rosterStatus:player.roster_status??null,ownerTeamId:player.owner_team_id??null,isFreeAgent:player.is_free_agent??player.owner_team_id===null,warning:"Authoritative player-level league production is not available."},
    ...
    role:roleEvidence,ageDevelopment:{status:player.age!==null&&player.age!==undefined?INPUT_AVAILABILITY.AVAILABLE:INPUT_AVAILABILITY.MISSING,age:player.age??null,isMinorLeaguer:Boolean(player.is_minor_leaguer),organization:player.mlb_team??null,level:null,readiness:null},
    prospectContext:{status:player.is_minor_leaguer?INPUT_AVAILABILITY.AVAILABLE:INPUT_AVAILABILITY.UNAVAILABLE,isProspect:Boolean(player.is_minor_leaguer),classification:null,rosterSlotUsed:Boolean(player.owner_team_id),readiness:null},replacementContext:{status:INPUT_AVAILABILITY.UNAVAILABLE,frontier:null,advantage:null},
    dataFreshness:{statcast:statcastFreshness,hkb:{status:hkbAvailable?INPUT_AVAILABILITY.UNAVAILABLE:INPUT_AVAILABILITY.MISSING,source:"HKB",season:null,fetchedAt:null,ageMs:null},fantrax:{status:INPUT_AVAILABILITY.UNAVAILABLE,source:"Fantrax",season:season??null,fetchedAt:null,ageMs:null}},warnings};
}
