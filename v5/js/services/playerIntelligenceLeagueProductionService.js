...
import {withProspectLevelSchemaState} from "./prospectLevelEvidence.js";
...
export async function loadLeagueProductionEvaluation({leagueId,season,asOfDate,repositories={},replacementTopN=5,chunkSize,yieldFn,onProgress,performanceMetrics}={}){
  ...
  const indexStarted=performance.now(),metricsByPlayerId=buildPlayerMetricIndex(metricRows,season),indexBuildMs=performance.now()-indexStarted,inputStarted=performance.now(),inputs=withProspectLevelSchemaState(players||[]).map(player=>buildCanonicalPlayerIntelligenceInput({player,league,metricsByPlayerId,season,asOfDate})),contextStarted=performance.now(),context=buildLeagueProductionContext(inputs,{replacementTopN}),contextBuildMs=performance.now()-contextStarted;
