...
export async function readProtectedBaselineDomains(leagueId,{dependencies={}}={}){
  if(!leagueId)throw new Error("Active league is required for protected baseline evidence.");
  const d={client,allPlayers,allTeams,listManagers,listMetrics,listScores,...dependencies};
  ...
  const [players,scores,metrics,teams,managers]=await Promise.all([
    d.allPlayers(leagueId),d.listScores(leagueId),d.listMetrics(leagueId),d.allTeams(leagueId),d.listManagers(leagueId)
  ]);
  return {players,scores,metrics,teams,managers};
}
