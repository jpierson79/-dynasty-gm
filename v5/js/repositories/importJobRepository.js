...
export async function startFantraxProductionJob(leagueId,{fileName,sourceMetadata}={}){
  if(!leagueId)throw new Error("Active league is required.");
  const supabase=await client(),auth=await supabase.auth.getUser();
  if(auth.error||!auth.data?.user?.id)throw new Error("Sign in before importing Fantrax production.");
  const startedAt=new Date().toISOString();
  const result=await request(supabase.from("import_jobs").insert({league_id:leagueId,user_id:auth.data.user.id,import_type:"fantrax_player_production",file_name:fileName||null,status:"running",started_at:startedAt,source_metadata:sourceMetadata||{}}).select("*").single(),"Fantrax production import job insert");
  return result.data;
}

export async function finishFantraxProductionJob(leagueId,jobId,{status,processed=0,matched=0,unmatched=0,inserted=0,updated=0,failed=0,errors=[],errorMessage=null,sourceMetadata={}}={}){
  if(!leagueId||!jobId)throw new Error("Active league and Fantrax production import job are required.");
  const supabase=await client();
