...
function personRow(person,sportId){
  ...
  return {mlbamId:String(id),fullName,active:person.active!==false,birthDate:clean(person.birthDate),sportId:Number(sportId),primaryPosition:clean(person.primaryPosition?.abbreviation||person.primaryPosition?.code),currentTeam:{id:Number(team.id)||null,name:clean(team.name),abbreviation:clean(team.abbreviation),teamCode:clean(team.teamCode),fileCode:clean(team.fileCode),parentOrgId:Number(team.parentOrgId)||null}};
}
function mergePerson(current,next){
  ...
  return {...current,...preferred,active:current.active||next.active,sportIds:[...new Set([...(current.sportIds||[]),next.sportId])].sort((a,b)=>a-b)};
}
...
export async function fetchMlbIdentityCatalog({season=new Date().getUTCFullYear(),sportIds=DEFAULT_SPORT_IDS,fetchImpl=globalThis.fetch,timeoutMs=DEFAULT_TIMEOUT_MS}={}){
  ...
  return {provider:"MLB Stats API",season:year,fetchedAt:new Date().toISOString(),sportIds:sportIds.map(Number),teams,people:[...people.values()].sort((a,b)=>Number(a.mlbamId)-Number(b.mlbamId)),invalid,warnings:invalid.map(row=>({reason:row.reason,sportId:row.sportId}))};
}
