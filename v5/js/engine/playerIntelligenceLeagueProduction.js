...
import {buildExactPercentileContext,exactPercentile} from "./playerIntelligencePercentile.js";
...
export function buildLeagueProductionContext(inputs,{replacementTopN=5}={}){
  ...
  const productionPercentiles=buildExactPercentileContext(productionValues);
  return {schema:"player-intelligence-league-context-v1",version:LEAGUE_PRODUCTION_VERSION,leagueId,season,inputsById:new Map(inputs.map(input=>[input.playerId,input])),rows,productionValues,productionPercentiles,productionRange:productionPercentiles.range,demand,pools,availableByPosition,replacementTopN};
}
...
export function evaluateLeagueProductionPlayer(context,playerId){
  ...
  const comparisons=positions.map(position=>{const replacement=replacementFor(context,position,playerId);return {position,playerFantasyPoints:points,replacementFantasyPoints:replacement.production,rawGap:numeric(replacement.production)?round(points-replacement.production):null,availableCompared:replacement.count}}),valid=comparisons.filter(row=>numeric(row.rawGap)).sort((a,b)=>b.rawGap-a.rawGap),best=valid[0]||null,range=context.productionRange;
