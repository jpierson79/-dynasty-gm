...
const emptyInspectionState=()=>({status:"NOT_LOADED",contextKey:"",result:null,rows:[],ranking:"expected",filters:{group:"",archetype:"",position:"",availability:"",marketDivergence:"",missingStatcast:false},page:1,pageSize:50,totalMatchingRows:0,totalPages:0,compareSearchA:"",compareSearchB:"",compareSuggestionsA:[],compareSuggestionsB:[],selectedPlayer:null,compareA:"",compareB:"",comparison:null,evaluatedPlayers:0,totalPlayers:0,progressPercent:0,currentPhase:"",elapsedMs:0,durationMs:null,error:""});
...
function refreshInspectionRows(){const inspection=inspectionUi.result?.inspection;if(!inspection)return;let filtered=filterInspectionPlayers(inspection,{archetype:inspectionUi.filters.archetype||undefined,position:inspectionUi.filters.position||undefined,availability:inspectionUi.filters.availability||undefined,marketDivergence:inspectionUi.filters.marketDivergence||undefined,missingStatcast:inspectionUi.filters.missingStatcast?true:undefined});if(inspectionUi.filters.group)filtered=filtered.filter(row=>row.groups.includes(inspectionUi.filters.group));const ranked=rankInspectionPlayers({...inspection,players:filtered},{dimension:inspectionUi.ranking}),totalMatchingRows=ranked.length,totalPages=Math.max(1,Math.ceil(totalMatchingRows/inspectionUi.pageSize)),page=Math.min(Math.max(1,inspectionUi.page),totalPages),start=(page-1)*inspectionUi.pageSize;inspectionUi={...inspectionUi,page,totalMatchingRows,totalPages,rows:ranked.slice(start,start+inspectionUi.pageSize),comparison:null};setState({playerIntelligenceInspection:inspectionUi})}
...
function bindViewEvents(){
  ...
  $("#inspectionPreviousPage")?.addEventListener("click",()=>{inspectionUi={...inspectionUi,page:Math.max(1,inspectionUi.page-1)};refreshInspectionRows()});
  $("#inspectionNextPage")?.addEventListener("click",()=>{inspectionUi={...inspectionUi,page:Math.min(inspectionUi.totalPages,inspectionUi.page+1)};refreshInspectionRows()});
  $("#inspectionPageSize")?.addEventListener("change",event=>{inspectionUi={...inspectionUi,page:1,pageSize:Number(event.target.value)};refreshInspectionRows()});
  const updateCompareSearch=(side,value)=>{const query=String(value||"").trim().toLowerCase(),players=inspectionUi.result?.inspection?.players||[],suggestions=query?players.filter(row=>row.identity.name?.toLowerCase().includes(query)||row.playerId.toLowerCase().includes(query)).slice(0,20):[];inspectionUi={...inspectionUi,[`compareSearch${side}`]:value,[`compareSuggestions${side}`]:suggestions,comparison:null};setState({playerIntelligenceInspection:inspectionUi})};
  $("#inspectionCompareSearchA")?.addEventListener("input",event=>updateCompareSearch("A",event.target.value));
  $("#inspectionCompareSearchB")?.addEventListener("input",event=>updateCompareSearch("B",event.target.value));
  $all("[data-inspection-compare]").forEach(button=>button.addEventListener("click",()=>{const side=button.dataset.inspectionCompare,id=button.dataset.playerId;inspectionUi={...inspectionUi,[`compare${side}`]:id,[`compareSearch${side}`]:inspectionUi.result.inspection.players.find(row=>row.playerId===id)?.identity.name||id,[`compareSuggestions${side}`]:[],comparison:null};setState({playerIntelligenceInspection:inspectionUi})}));
  ...
  $("#captureMlbamProtectedBaseline")?.addEventListener("click",async()=>{
    const current=importUiState.mlbamBackfill,protectedBaseline={running:true,evidence:null,error:""};importUiState.mlbamBackfill={...current,protectedBaseline};render();
    const evidence=await captureProtectedBaseline({leagueId:appState.activeLeague?.id,profile:"MLBAM_BACKFILL"});
    importUiState.mlbamBackfill={...importUiState.mlbamBackfill,protectedBaseline:{running:false,evidence,error:evidence.status==="AVAILABLE"?"":evidence.errors?.join(" ")||evidence.status}};render();
  });
  $("#captureStatcastProtectedBaseline")?.addEventListener("click",async()=>{
    const current=importUiState.statcast,protectedBaseline={running:true,evidence:null,error:""};importUiState.statcast={...current,protectedBaseline};render();
    const evidence=await captureProtectedBaseline({leagueId:appState.activeLeague?.id,profile:"STATCAST_REFRESH"});
    importUiState.statcast={...importUiState.statcast,protectedBaseline:{running:false,evidence,error:evidence.status==="AVAILABLE"?"":evidence.errors?.join(" ")||evidence.status}};render();
  });
